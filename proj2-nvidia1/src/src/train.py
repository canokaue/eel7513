...
if __name__ == '__main__':
    ...
    cropped_train = torch.utils.data.random_split(train_dataset, [const.TRAIN_SPLIT_LEN, len(train_dataset)-const.TRAIN_SPLIT_LEN])[0]
    train_dataloader = torch.utils.data.DataLoader(cropped_train, batch_size=const.BATCH_SIZE, shuffle=True, num_workers=const.NUM_WORKERS)
    ...
    cropped_val = torch.utils.data.random_split(val_dataset, [const.VAL_SPLIT_LEN, len(val_dataset)-const.VAL_SPLIT_LEN])[0]
    val_dataloader = torch.utils.data.DataLoader(cropped_val, batch_size=const.VAL_BATCH_SIZE, shuffle=False, num_workers=const.NUM_WORKERS)
